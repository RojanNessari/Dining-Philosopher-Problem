...
public class Philosopher extends BaseThread
{
	...
	public void eat() {
	    try {
	        ...
	        Thread.yield(); // Yield to encourage context switching
	        sleep((long) (Math.random() * TIME_TO_WASTE)); // Simulate eating
	        Thread.yield();
	...
	public void think() {
	    try {
	        ...
	        Thread.yield();
	        sleep((long) (Math.random() * TIME_TO_WASTE)); // Simulate thinking
	        Thread.yield();
	...
	public void talk() {
	    try {
	        DiningPhilosophers.soMonitor.requestTalk(); // Request permission to talk
	        System.out.println("Philosopher " + getTID() + " starts talking.");
	        saySomething(); // Print a random phrase
	        Thread.yield();
	        System.out.println("Philosopher " + getTID() + " finishes talking.");
	        DiningPhilosophers.soMonitor.endTalk(); // Release talking permission
	    } catch (Exception e) {
	...
	public void run()
	{
		for(int i = 0; i < DiningPhilosophers.DINING_STEPS; i++)
		{
			 ...
			 */
			if (Math.random() < 0.3) { // 30% chance to talk
	            talk();
	        }
			BaseThread.yield();
		}
